...
class ProgressNet(nn.Module):
    ...
    def forward(self, frames: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # x = torch.arange(1, S+1, 1, dtype=torch.float32, device=device, requires_grad=True) / 128.31116767419664
        # x = torch.ones(B, S, dtype=torch.float32, device=device, requires_grad=True)
        x = torch.rand((B, S), dtype=torch.float32, device=device, requires_grad=True)

        x = x.reshape(num_samples, -1)
        embedded = torch.relu(self.fc7(x))
        embedded = embedded.reshape(B, S, -1)
        # forecast
        forecasted_embedded = torch.ones_like(embedded, dtype=torch.float32, requires_grad=True, device=device)

        # lstm
        progress, _ = self.lstm1(embedded)
        progress, _ = self.lstm2(progress)
        ...
        return progress, torch.ones_like(progress, dtype=torch.float32, requires_grad=True, device=device), embedded, forecasted_embedded
