...
from networks import ProgressNetMHN
...
def train(network, batch, l1_criterion, l2_criterion, device, optimizer=None):
    ...
    if optimizer:
        optimizer.zero_grad()
    predictions_1, predictions_2, predictions_3 = network(frames, boxes, lengths)
    # progress is in range (0, 1], but batch is zero-padded
    # we can use this to fill our loss with 0s for padded values
    predictions_1.masked_fill_(labels == 0, 0.0)
    predictions_2.masked_fill_(labels == 0, 0.0)
    predictions_3.masked_fill_(labels == 0, 0.0)

    l1_loss_1 = l1_criterion(predictions_1, labels)
    l1_loss_2 = l1_criterion(predictions_2, labels)
    l1_loss_3 = l1_criterion(predictions_3, labels)

    l1_loss = torch.minimum(torch.minimum(l1_loss_1, l1_loss_2), l1_loss_3)
...
def main():
    ...
    # load model
    net = ProgressNetMHN(embed_size=args.embed_size, p_dropout=args.dropout_chance).to(device)
    if args.model_name:
    ...
    for epoch in range(args.epochs):
        ...
        if not args.eval:
            ...
            for batch in tqdm(train_loader, leave=False):
                l1_loss, count = train(net, batch, l1_criterion, l2_criterion, device, optimizer=optimizer)
            ...
            logging.info(f'[{epoch:03d} train]       avg l1 loss {(train_l1_loss / train_count):.4f}')
        ...
        for batch_index, batch in tqdm(enumerate(test_loader), leave=False, total=len(test_loader)):
            ...
            for model_name in networks:
                l1_loss, count = train(networks[model_name]['net'], batch, l1_criterion, l2_criterion, device)
        ...
        for model_name in networks:
            logging.info(f'[{epoch:03d} test {model_name}] avg l1 loss {(networks[model_name]["l1_loss"] / networks[model_name]["count"]):.4f}')
