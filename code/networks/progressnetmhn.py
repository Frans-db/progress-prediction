...
class ProgressNetMHN(nn.Module):
    def __init__(self, embed_size=4069, p_dropout=0, finetune=False):
        super(ProgressNetMHN, self).__init__()
        ...
        self.fc8_1 = nn.Linear(32, 1)
        self.fc8_2 = nn.Linear(32, 1)
        self.fc8_3 = nn.Linear(32, 1)
        self.fc8_1_dropout = nn.Dropout(p=p_dropout)
        self.fc8_2_dropout = nn.Dropout(p=p_dropout)
        self.fc8_3_dropout = nn.Dropout(p=p_dropout)

        self._init_weights(self.spp_fc)
        self._init_weights(self.roi_fc)
        self._init_weights(self.fc7)
        self._init_weights(self.fc8_1)
        self._init_weights(self.fc8_2)
        self._init_weights(self.fc8_3)
    ...
    def forward(self, frames, boxes, lengths):
        ...
        unpacked_1 = self.fc8_1_dropout(self.fc8_1(unpacked))
        unpacked_1 = unpacked_1.reshape(batch_size, sequence_length)

        unpacked_2 = self.fc8_2_dropout(self.fc8_2(unpacked))
        unpacked_2 = unpacked_2.reshape(batch_size, sequence_length)

        unpacked_3 = self.fc8_3_dropout(self.fc8_3(unpacked))
        unpacked_3 = unpacked_3.reshape(batch_size, sequence_length)

        return unpacked_1, unpacked_2, unpacked_3
