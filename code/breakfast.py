        ...
        for name in files:
            ...
            for index in testrange:
                if f'P{index:02d}' in path:
                    tests.append(path.replace(f'{walk_root}/', '').replace('.txt', ''))
                    found = True
                    break

            if not found:
                trains.append(path.replace(f'{walk_root}/', '').replace('.txt', ''))

    print(f'{splitname} train {len(trains)} test {len(tests)} total {len(trains) + len(tests)}')
    with open(f'splitfiles/train_{actions[0]}_{splitname}.txt', 'w+') as f:
        f.write('\n'.join(trains))
    with open(f'splitfiles/test_{actions[0]}_{splitname}.txt', 'w+') as f:
        f.write('\n'.join(tests))
